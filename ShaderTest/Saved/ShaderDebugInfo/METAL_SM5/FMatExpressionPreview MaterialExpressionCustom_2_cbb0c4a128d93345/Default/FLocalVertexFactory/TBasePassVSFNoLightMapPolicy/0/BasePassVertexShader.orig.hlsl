// DebugHash_27fdedc83957a0016f3d8e10a9277ea6000a023d6646c84052699f21b06dad08
...
float perlinNoise(vec2 p, float t)
{
    ...
    return mix(mix(bl, tl,  smoothstep(0., 1., fract(p.y) )), 
               mix(br, tr,  smoothstep(0., 1., fract(p.y) )),
                smoothstep(0., 1., fract(p.x) ))*0.5 + 0.5;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float v = perlinNoise(uv*2.+iDate.w*0.05, iDate.w*0.2);
    float fw = fwidth(v);
    float t = 0.8;
    vec3 col = (smoothstep(t, t+fw*10.*2., pulse(v*10., 1.))*0.8+0.2)*vec3(1.000,0.812,0.643);    col = mix(col.xyz, col.xyy, smoothstep(0., .25, sin(v*6.2831)));
