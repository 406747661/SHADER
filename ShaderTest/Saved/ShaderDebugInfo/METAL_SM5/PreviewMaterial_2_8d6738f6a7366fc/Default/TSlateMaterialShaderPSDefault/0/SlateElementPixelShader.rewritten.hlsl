// DebugHash_224d2fc5e89afd86e0503c85eb2c6aa2e941de5a9b1f4b060cde1d60b5d3ab96
...
_Pragma("dxc diagnostic push")
_Pragma("dxc diagnostic ignored \"-Wall\"") 					_Pragma("dxc diagnostic ignored \"-Wconversion\"") 				_Pragma("dxc diagnostic ignored \"-Wfor-redefinition\"") 		_Pragma("dxc diagnostic ignored \"-Winline-asm\"")				_Pragma("dxc diagnostic ignored \"-Wunsequenced\"")
float3 CustomExpression0(FMaterialPixelParameters Parameters,float2 uv,float time)
{
float4 Out = 0;
float2 r = uv;
float2 p = uv * 4 -2;    
return p;
float z = (4.-4.*abs(.7-dot(p,p)));
float2 i = 0;
float2 f = p * z;
for(Out = 0.; i.y++<8.;
Out += (sin(f)+1.).xyyx * abs(f.x-f.y))
f += cos(f.yx*i.y+i+time)/i.y+.7;
Out = tanh(7.*exp(z.x-4.-p.y*float4(-1,1,2,0))/Out);
return p;
}
_Pragma("dxc diagnostic pop")
...
{
	float2 Local3 = Parameters.TexCoords[0].xy;
	OutTexCoords[0] = Local3;
}
_Pragma("dxc diagnostic pop")
...
_Pragma("dxc diagnostic ignored \"-Wall\"") 					_Pragma("dxc diagnostic ignored \"-Wconversion\"") 				_Pragma("dxc diagnostic ignored \"-Wfor-redefinition\"") 		_Pragma("dxc diagnostic ignored \"-Winline-asm\"")				_Pragma("dxc diagnostic ignored \"-Wunsequenced\"")
	...
	float3 Local1 = CustomExpression0(Parameters,  Local0 ,View_GameTime);
	float3 Local2 = lerp(Local1,Material_PreshaderBuffer[0].xyz,Material_PreshaderBuffer[0].w);
...
float4 DrawFlags;
float4 ApplyContrast(float4 InColor)
{
    [flatten] if( GammaAndAlphaValues.w != 1.0f )
    {
		float ContrastMidpoint = 0.25; 
...
float4 GetMaterialColor( VertexToPixelInterpolants InVertex, float2 MeshUV, float4 InFontSignedDistanceData = float4(0.0, 0.0, 0.0, 0.0) )
{
...
{
	return GetColor( InVertex,   InVertex .TextureCoordinates0.xy *   InVertex .TextureCoordinates0.zw );
}
float4 Main( VertexToPixelInterpolants VIn ) : SV_Target0
{
	float4 OutColor;
	OutColor = GetDefaultElementColor( VIn );
	const float DrawDisabledEffect = DrawFlags.x;
